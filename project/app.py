...
##### For tag research:
    ...
    if request.args.get("tag1") in ANIME_TAGS and request.args.get("year") and request.args.get("rate"):
        ...
        rate = request.args.get("rate")
        # Sqlite query with tag1, tag2, tag3, year, rate
        anime = db.execute("SELECT * FROM anime WHERE Premiered LIKE ? OR Premiered = 'UNKONW' AND Score > ? AND Genres LIKE ? AND Genres LIKE ? AND Genres LIKE ? ORDER BY RANDOM() LIMIT 10",
